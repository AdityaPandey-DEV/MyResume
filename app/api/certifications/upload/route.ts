...
export async function POST(request: NextRequest) {
    try {
        ...
        if (process.env.BLOB_READ_WRITE_TOKEN) {
            const blob = await put(`certifications/${fileName}`, file, {
                access: 'public',
            })
        ...
        const uploadDir = join(process.cwd(), 'public', 'images', 'certifications')
        ...
        const imageUrl = `/images/certifications/${fileName}`
        revalidatePath('/')
